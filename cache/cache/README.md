...
The cache should be an FSM with 4 states:

- IDLE: The default state. Wait for commands from CPU.

- HIT: Read/write hit. Return the read data or update the written data if hit.

- WRITEBACK: Dirty miss state. Flush dirty data from cache to memory.

- ALLOCATE: Read data from memory to cache.
